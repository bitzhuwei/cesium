...
define([
    ...
    function requestTile(tile) {
        // A Cesium3DTile will not be able to create its Content right away. This actually might work ok with expiration / ready promises.
        //
        // The content returned may be json
        // If it has an extension, use that because it helps whether it is json or not.
        // If that isn't known, it might just have to load an array buffer and see if its json.
        // The content
    }
    ...
    Cesium3DTileset.prototype.loadTileset = function(tilesetUrl, tilesetJson, parentTile) {
        ...
        while (stack.length > 0) {
            ...
            if (defined(requestServer)) {
                if (!defined(this._loadHeaps[requestServer])) {
                    var heap = new Heap(sortForLoad);
                    this._loadHeaps[requestServer] = heap;
                    heap.maximumSize = RequestScheduler.maximumRequestsPerServer;
                    heap.reserve(heap.maximumSize);
                }
                tile3D._loadHeap = this._loadHeaps[requestServer];
            }
        }
